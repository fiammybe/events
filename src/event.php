 ...
 * Event page
 *
 * @copyright    Madfish (Simon Wilkinson) 2013.
...
if ($eventObj && ! $eventObj->isNew()) {
    ...
    if (icms_get_module_status("sprockets")) {
        $event_tags = array();
        $event_tag_array = $sprockets_taglink_handler->getTagsForObject(
            $eventObj->getVar('event_id'),
            $events_event_handler,
            0
        );
        foreach ($event_tag_array as $value) {
            $event_tags[] = '<a href="' . EVENTS_URL . 'event.php?tag_id=' . $value . '">' . $sprockets_tag_buffer[$value] . '</a>';
        }
    ...
    $icmsTpl->assign(
        "events_event",
        $events_event_handler->prepareEventForDisplay($eventObj, true)
    );

    $icms_metagen = new icms_ipf_Metagen(
        $eventObj->getVar("title"),
        $eventObj->getVar("meta_keywords", "n"),
        $eventObj->getVar("meta_description", "n")
    );
    ...
    if ($eventObj->getVar('image')) {
        $xoTheme->addMeta(
            'meta',
            'og:image',
            ICMS_URL . '/uploads/' . basename(dirname(__FILE__, 1)) . '/article/' . $eventObj->getVar('image')
        );
    }
    ...
    if (icms_get_module_status("sprockets")) {
        ...
        if (icms::$module->config['events_show_tag_select_box']) {
            if ($untagged_content) {
                $tag_select_box = $sprockets_tag_handler->getTagSelectBox(
                    'event.php',
                    'untagged',
                    _CO_EVENTS_ALL_TAGS,
                    true,
...
$icmsTpl->assign(
    'events_module_home',
    '<a href="' . ICMS_URL . "/modules/" . icms::$module->getVar("dirname") . '/">' . icms::$module->getVar("name") . "</a>"
);
