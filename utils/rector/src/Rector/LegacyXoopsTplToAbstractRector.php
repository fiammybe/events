...
/**
 * Transforms legacy $xoopsTpl variable usage to $icmsTpl variable.
 *
 * @see \Utils\Rector\Tests\Rector\LegacyXoopsTplToAbstractRector\LegacyXoopsTplToAbstractRectorTest
 */
final class LegacyXoopsTplToAbstractRector extends AbstractRector
{
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition('Transform $xoopsTpl variable to $icmsTpl variable', [
            new CodeSample(
                <<<'CODE_SAMPLE'
$xoopsTpl->assign('title', 'Page Title');
if ($xoopsTpl) {
    $xoopsTpl->display('template.html');
}
CODE_SAMPLE
                ,
                <<<'CODE_SAMPLE'
$icmsTpl->assign('title', 'Page Title');
if ($icmsTpl) {
    $icmsTpl->display('template.html');
}
CODE_SAMPLE
            )
        ]);
    ...
    private function transformVariable(Node $subNode, bool &$hasChanged): ?Node
    {
        ...
        if (!$this->isName($subNode, 'xoopsTpl')) {
            return null;
        }
