...
/**
 * Transforms legacy $xoopsUser and $icmsUser variable usage to icms::$user static property access.
 *
 * @see \Utils\Rector\Tests\Rector\LegacyXoopsUserToAbstractRector\LegacyXoopsUserToAbstractRectorTest
 */
final class LegacyXoopsUserToAbstractRector extends AbstractRector
{
    public function getRuleDefinition(): RuleDefinition
    {
        return new RuleDefinition('Transform $xoopsUser and $icmsUser to icms::$user static property access', [
            new CodeSample(
                <<<'CODE_SAMPLE'
if ($xoopsUser) {
...
CODE_SAMPLE
    ...
    private function transformVariable(Node $subNode, bool &$hasChanged): ?Node
    {
        ...
        if (!$this->isName($subNode, 'xoopsUser') && !$this->isName($subNode, 'icmsUser')) {
            return null;
        }
        ...
        $hasChanged = true;
        return new StaticPropertyFetch(new Name('icms'), 'user');
    }
}
